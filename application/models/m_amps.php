...
class M_Amps extends CI_Model {
   ...
   function get_avg_amps_by_period($site_id, $date, $period)  // used for charts
    {
        switch($period)
        {
            ...
            case 'day':
             $sql = "select b.hour, IFNULL(round(avg(a.out_current), 1),0.00) as out_current, IFNULL(round(avg(a.batt_v), 1),0.00) as batt_v  
                    from number b LEFT OUTER JOIN consolidatedexport a 
                    ON b.hour = HOUR(CONVERT_TZ(a.created_date, '$this->server_TZ', '$this->site_TZ')) and a.site_id=$site_id and
                    DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ'))='" .  $date . "' GROUP BY b.hour";
            
            break;

            case 'week':
           
            $sql = "select DATE_FORMAT(b.fulldate, '%c/%e') as date, IFNULL(round(avg(a.out_current), 1),0.00) as out_current, IFNULL(round(avg(a.batt_v), 1),0.00) as batt_v
            ...
            $sql = "select DATE_FORMAT(b.fulldate, '%c/%e') as date, IFNULL(round(avg(a.out_current), 1),0.00) as out_current, IFNULL(round(avg(a.batt_v), 1),0.00) as batt_v
                                    from consolidatedexport a RIGHT JOIN datedim b ON DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) = b.fulldate
                     and a.site_id=$site_id where b.fulldate <= '$date'and b.fulldate >= '$this->golivedate' group by b.fulldate limit 365";
            break;
    ...
    function get_amp_detail($site_id, $date, $period)  // used for the period detail
    {
        switch($period)
        {
            ...
            $sql = "select DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) as date, HOUR(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) as hour,
                  round(avg(batt_v), 1) as batt_v, round(avg(out_current), 1) as out_current, round(max(out_kwh), 1) as out_kwh 
            ...
            $sql = "select DATE_FORMAT(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ'), '%c/%e/%y') as date, DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) as day,
                    round(avg(batt_v), 1) as batt_v, round(avg(out_current), 1) as out_current, 
                    round(max(out_ah), 1) as out_ah from consolidatedexport where site_id=$site_id and 
            ...
            $sql = "select DATE_FORMAT(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ'), '%c/%e/%y') as date, DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) as day,
                    ...
                    round(max(out_ah), 1) as out_ah from consolidatedexport where site_id=$site_id and 
                    DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) <= '$date' AND 
                    DATE(CONVERT_TZ(created_date, '$this->server_TZ', '$this->site_TZ')) >= '$this->golivedate' group by date order by day desc";
            break;
