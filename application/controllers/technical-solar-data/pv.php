...
class Pv extends MY_Controller {
	...
	public function index()
	{
		...
		redirect('/technical-solar-data/pv/day');
	...
	public function day($date = null)
	{
		...
		{
			$day= $date;
			$api_url = "api/pv/day/date/" . $day . "/siteID/" . $this->sitePubID; 
			$today = "false";
		}
		else
		{
		...
		$data['pv'] = $this->m_pv->get_pv_detail($this->site_id, $day, 'day');
		
		$data['period'] = 'day';
		$data['page'] = 'solardata/v_pv';
		$data['chartdatelabel'] = myDateInterval($day, 'day');
		$data['api']= array("url" => $api_url, 
							"success" => "grabAndFillPV(data, 'hour', $today);",
	...
	public function week($date = null)
	{
		...
		{
			$day= $date;
			$api_url = "api/pv/week/date/" . $day . "/siteID/" . $this->sitePubID; 
		}
		else
		{
			// default to today()
		...
		$data['pv'] = $this->m_pv->get_pv_detail($this->site_id, $day, 'week');
		
		$data['period'] = 'week';
		$data['page'] = 'solardata/v_pv';
		$data['chartdatelabel'] = myDateInterval($day, 'week');
		$data['api']= array("url" => $api_url, 
							"success" => "grabAndFillPV(data, 'week');",
	...
	public function month($date = null)
	{
		...
		{
			$day= $date;
			$api_url = "api/pv/month/date/" . $day . "/siteID/" . $this->sitePubID; 
		}
		else
		{
			// default to today()
		...
		$data['pv'] = $this->m_pv->get_pv_detail($this->site_id, $day, 'month');
		
		$data['period'] = 'month';
		$data['page'] = 'solardata/v_pv';
		$data['chartdatelabel'] = myDateInterval($day, 'month');
		$data['api']= array("url" => $api_url, 
							"success" => "grabAndFillPV(data, 'month');",
	...
	public function year($date= null)
	{
		...
		{
			$day= $date;
			$api_url = "api/pv/year/date/" . $day . "/siteID/" . $this->sitePubID; 
		}
		else
		{
			// default to today()
		...
		$data['pv'] = $this->m_pv->get_pv_detail($this->site_id, $day, 'year');
		
		$data['period'] = 'year';
		$data['page'] = 'solardata/v_pv';
		$data['chartdatelabel'] = myDateInterval($day, 'year');
		$data['api']= array("url" => $api_url, 
							"success" => "grabAndFillPV(data, 'year');",
	...
	{
		$day = date("Y-m-d"); 
		$api_url = "api/pv/since_inception/siteID/" . $this->sitePubID; 

		$this->load->model('m_pv');
		
		$data['pv'] = $this->m_pv->get_pv_detail($this->site_id, $day, 'inception');
		
		$data['period'] = 'inception';
		$data['page'] = 'solardata/v_pv';
		$data['chartdatelabel'] = myDateInterval($day, 'inception');
		$data['api']= array("url" => $api_url, 
							"success" => "grabAndFillPV(data, 'inception');",
