...
Raphael.fn.draw_HGrid = function(x, y, width, height, x_step, x_size, y_count, y_size) {
	
	//start path with line at 0
	
	var path = "";
	path = path.concat(["M", Math.round(x) + .5, 290, "H", Math.round(x + width) + 0.5]);
	//var ypathstring = 'M ' + canvas_x_chart_offset + ' ' + canvas_y_offset + ' l' + ' 0 ' + (chartheight - canvas_y_offset);
	var	rowHeight;
	var columnWidth;
	...
	for (var i = 0; i < y_count; i++) {
		path = path.concat(["M", Math.round(x) + .5, Math.round(y + i * rowHeight) + 0.5,
...
Raphael.fn.lineChart = function(method) {
	...
	var methods = {
		...
		init: function(options) {
			...
			// draw horizontal grid regardless of setting
			if (!o.gridDrawn && settings.no_grid === false) {
				var grid = element.draw_HGrid(gutter.left + X * 0.5 + 0.5,
								gutter.top + 0.5,
								width - gutter.left - X,
								...
								max)
							.attr({ stroke: "#b2b7a0", 'stroke-width': "2" });
			...
			/* // we're altering this for our needs -- Kevin
			if (!o.gridDrawn && settings.no_grid === false) {
			...
			*/
			...
			if (settings.y_labels_count) {
				lside = settings.y_label_side;
				helpers.drawYLabels(element, max, lside);
			}
			...
			o.label.push(element.text(60, 27, "22 September 2008").attr(settings.text.popup_line2));
			/*
			...
			*/
			...
			for (var i = 0, ii = datasize; i < ii; i++) {
				var dot, rect;
	...
	var helpers = {
		...
		drawYLabels: function(elm, max, lside) {
...
Raphael.fn.lineChart.defaults = {
	...
	y_label_side: 'l',   // in case we put two lines on the chart.
